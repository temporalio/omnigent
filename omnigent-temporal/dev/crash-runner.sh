...
pid_of worker >/dev/null || die "no worker recorded. Start dev/worker.sh"
HOST_PID="$(pid_of host)" || die "no host recorded. Start dev/omnigent.sh"
...
# Only a runner under our own host, found by walking its process tree. Matching
# on the process name would also kill a runner belonging to another Omnigent on
# this machine.
RUNNER_PIDS="$(descendants "$HOST_PID" | tr '\n' ' ')"
log "host pid $HOST_PID"

HOST_PID="$HOST_PID" "$PY" - "$KEY" <<'PY'
import asyncio, contextlib, os, signal, subprocess, sys, uuid
...
def our_runners(host_pid: int) -> list[int]:
    """Runner processes under our own host, so another Omnigent on this machine is
    left alone."""
    found: list[int] = []
    frontier = [host_pid]
    while frontier:
        ...
        for child in children:
            ...
            frontier.append(pid)
            cmd = subprocess.run(
                ["ps", "-o", "command=", "-p", child], capture_output=True, text=True
            ).stdout
            if "omnigent.runner" in cmd:
                found.append(pid)
    return found
...
async def main() -> int:
    ...
    # Wait for our own host to have a runner up, then take that one out.
    host_pid = int(os.environ["HOST_PID"])
    for _ in range(120):
        runners = our_runners(host_pid)
        if runners:
            break
        await asyncio.sleep(1)
    else:
        print("no runner ever started under our host", file=sys.stderr)
        return 1

    await asyncio.sleep(4)  # let the turn get going
    print(f"killing our runner(s): {runners}")
    for pid in runners:
